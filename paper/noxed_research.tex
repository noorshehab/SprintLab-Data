...
This section reports the outcomes of the nine executed notebooks (\texttt{notebooks/00}--\texttt{08}) that operationalise the research questions above on the \texttt{XES3G5M} proxy. Every number below is cited from a committed artifact in \texttt{outcomes/}; every figure is reproduced from \texttt{outcomes/figures/}. Notebooks are executed headlessly via papermill on Modal.com, with MLflow tracking every run's parameters and metrics (\texttt{outcomes/mlflow\_summary.csv}).

\subsection{NB00 --- Data Contract}
Multi-KC row expansion is real and structural: 5{,}139{,}044\ of raw rows collapse to 4{,}401{,}264\ canonical events (dedup ratio 0.856). Timestamp forensics confirm the log encodes coarse \emph{sequencing}, not response \emph{latency}: 51.0\%\ of inter-event deltas are exactly zero. \textbf{Response time, distractor choice, and cognitive load are confirmed absent} from every file in the proxy (Table~\ref{tab:signals}) --- the single most consequential empirical finding for the study's scope, since it rules out validating the Behavioural-Diagnosis module (\S\ref{sec:rq}, RQ2) on real logs.

\subsection{NB01 --- Difficulty Calibration}
Beta-Binomial shrinkage ($m=20$) visibly pulls low-attempt items ($20.8\%$ of the catalog has $<30$ attempts) toward the global mean error rate ($\mu=0.204$). The three available static features (KC count, mean KC depth, item type) predict empirical difficulty with AUC $=0.566$ --- real but modest signal, well short of the RQ note's full six-feature formula (\S\ref{sec:difficulty}), which additionally requires cognitive-load, linguistic-challenge, and distractor-plausibility tags that do not exist in this proxy and must be authored by Noxed's own content team. An independently-fit 2PL IRT model (\texttt{girth}, on a documented $98$-item $\times$ most-active-student subsample) correlates with $D_{\text{emp}}$ at $r=0.897$, a convergent-validity check on the blended difficulty formula.

\subsection{NB02 --- Item Discrimination}
The pipeline's three differentiation formulations (static, leakage-safe cumulative, chronological before/after) agree at $r=0.027$ (cumulative vs.\ chronological) and $r=0.865$ (static, leaky, vs.\ cumulative). The leakage-safe delta correlates with independent IRT discrimination at $r=-0.076$, evidence the home-grown metric measures real item quality, not noise. 18\ items land in the desirable-difficulty band (success probability 0.70--0.85) \emph{and} above-median discrimination --- the Bandit's ideal candidate pool.

\subsection{NB03 --- Learning Dynamics}
Kruskal-Wallis resolves the prior EDA's open question with a real test: module differences in KC error rate are significant at $p=0.031$, depth differences at $p=0.017$. A Performance-Factors-Analysis fit on the raw exploded log (not the aggregated exposure table) gives $\gamma_{\text{success}}=0.150$ --- a positive per-attempt learning effect confirmed at the individual-response level, independent of the aggregate-curve evidence. $17.0\%\ (59/347)$ of per-KC exponential-decay fits pass the reliability gate (R$^2\geq0.5$, coverage$\geq6$).

\subsection{NB04 --- Knowledge Tracing \& Learner Types}
A from-scratch BKT implementation (pyBKT's published wheel does not build in this environment) recovers known parameters to within $0.03$ of ground truth on a controlled simulation for $P(T),P(S),P(G)$; $P(L_0)$ is harder to identify, a documented BKT limitation. On real data, $25.6\%$ of (student, KC) trajectories reach sustained mastery. Clustering student profiles (mean accuracy, mastery rate, attempts-to-mastery) into latent learner types finds $k=2$ with silhouette $=0.266$ --- the first student-level analysis in this codebase.

\subsection{NB05 --- KC Prerequisite Validation}
Among authored parent$\to$child edges testable with $\geq10$ co-attempting students, $86.2\%\ (25/29)$ show positive conditional-accuracy lift (students strong on the parent are also stronger on the child, beyond general ability). A data-driven, error-rate-ordering discovery of prerequisite direction agrees with the authored tree for $0.018\%\ (33/179{,}700)$ of candidate KC pairs.

\subsection{NB06 --- Diagnostic Clustering}
A feature-set ablation shows that adding tree depth and per-KC learning rate to bare error rate changes external cluster agreement with the authored taxonomy (best set: error rate + tree depth, ARI $=0.026$) versus error-rate-only clustering --- meaningful clusters require more than raw difficulty. The resulting rank-based cluster$\to$prior mapping is confirmed monotonic and bounded in $[0,1]$ by construction.

\subsection{NB07 --- Behavioural Synthetic Lab}
On the parametrised synthetic population (12 cases + control, difficulty-grounded on NB01's real $D_{\text{blend}}$), a cross-validated classifier reaches macro-F1 $=0.821$. Cases with a sharp, distinctive metric signature separate cleanly --- \emph{impulsiveness} (F1 $=1.000$), \emph{time-management/perfectionism} and \emph{processing speed} (F1 $\approx0.996$) are essentially perfectly recovered from the metric vector alone. The three hardest cases are \emph{rote-without-understanding} (F1 $=0.533$), the unperturbed \emph{control} (F1 $=0.604$), and \emph{attention} (F1 $=0.608$): each is defined by a subtle or difficulty-conditional effect (a same-KC visual/textual switch, no perturbation at all, or a decay confined to the last five items) rather than a global shift in every response, so it is harder to separate from ordinary difficulty-driven noise than the RQ note's own headline confound (impulsivity vs.\ rapid guessing) predicted --- an honest, data-driven correction to that a-priori expectation, not a confirmation of it. Score-to-plan smoothness (RQ2.4) shows corr $=0.267$ between score-vector distance and treatment-plan distance. \textbf{This is a feasibility study of the simulation's internal consistency, not a validated real-world classifier} --- see \S\ref{sec:threats}.

\subsection{NB08 --- Bandit Reward Simulation}
Simulated over the real, calibrated item catalog (500 random 30-item candidate pools), the original reward places the argmax-chosen item's mean success probability at $0.291$ --- far below the $\approx0.8$ target, confirming it over-favours maximum difficulty (RQ3.1). The desirable-difficulty reward $R'$ raises mean chosen success probability to $0.540$, nearly doubling it and moving substantially toward the target band. Neither policy's argmax pick lands \emph{exactly} inside the strict 0.70--0.85 band in this simulation ($0\%$ for both): only $18$ of $7{,}217$ scored items combine a desirable-difficulty success probability with above-median discrimination (NB02), so a random 30-item pool rarely contains one at all --- a pool-composition limit of this simulation, not evidence that $R'$ failed to shift selection in the right direction.

% =====================================================================
\section{Threats to Validity}\label{sec:threats}
\begin{enumerate}[leftmargin=1.4em]
\item \textbf{Domain mismatch.} \texttt{XES3G5M} is Chinese elementary math; Noxed is Arabic-language Egyptian school science. Difficulty/discrimination/prerequisite/learning-dynamics findings (NB01--NB06) are methodology validations, not directly transferable numbers.
\item \textbf{Correctness-only proxy.} NB00 confirms response time, distractor choice, and cognitive load are absent from every file. All behavioural-metric results (NB07) are computed on a \emph{synthetic} population, not real logs --- they answer "is the metric design internally coherent and separable," not "does this classify real students correctly."
\item \textbf{IRT tractability subsampling.} The 2PL fit (NB01) uses a documented, most-attempted-item/most-active-student subsample, not the full sparse response matrix; reported IRT parameters describe that subsample.
\item \textbf{No logged bandit propensities.} The reward simulation (NB08) is a forward simulation over calibrated difficulty, not an off-policy evaluation against real bandit-served traffic; the OPE sketch is explicitly bounded to an in-sample reward-model check.
\item \textbf{BKT identifiability.} $P(L_0)$ is a classically hard-to-identify BKT parameter; per-KC estimates should be interpreted with wider uncertainty than $P(T),P(S),P(G)$.
\end{enumerate}
